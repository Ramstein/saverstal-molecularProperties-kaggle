...
for fileName in fileNames:
    if os.path.isfile(os.path.join(path, fileName)):
        if not os.path.isdir(os.path.join(path, fileName.split('.')[0])):
            ...
            print('Directory ',os.path.join(path, fileName.split('.')[0]),' has been created.')
        ...
        else:  print('Directory ',os.path.join(path, fileName.split('.')[0]),' is already available.')
    else:
...
'''!pip install /content/pretrainedmodels-0.7.4.tar > /dev/null'''
# package_path = '/content/model.py'
# import sys
...
def postProcessing(probability, threshold, min_size):
    '''
    processign the for the post work
    :param probability: probability of the prediction
    :param threshold: thrreshold upto which you want the prediction to be correct
    :param min_size: minimum size of the processing speed
    :return: prediction, and num
    '''
...
# listing the whole kit available in the unetstartermodelfile
...
testset = DataLoader(
    TestDataset(test_img, df, mean, std),
...
ckpt_path = '/content/model/model.pth'
